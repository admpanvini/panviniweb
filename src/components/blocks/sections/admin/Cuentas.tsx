...
import { CircleCheck, Edit, Trash, User, File, Bell, User2Icon } from "lucide-react";
...
export default function Cuentas() {
  ...
  async function handleConfirmEstado() {
    ...
    setErrorMsg("");
    setLoading(true)
    setLoadingText("Guardando estado de la cuenta..")
    const row = modal.row;
    const res = await fetch("/api/admin/savecuenta", {
      ...
      body: JSON.stringify({
        ...
        cuenta_unidad_codigo: row.cuenta_unidad_codigo,
        propiedad_codigo: row.cuenta_unidad_codigo?.substring(0, 2) || "",
  ...
  return (
    ...
    :(
      <div className="text-[var(--baseOscura-admin)]">
          ...
          <ConfirmModal
            open={modal.open}
            text={`¿Seguro que querés cambiar el estado de esta cuenta a ${modal.estado}?`}
